      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-3xl sm:text-4xl md:text-5xl font-bold text-charcoal font-signika leading-tight"
          >
        ...
        <div className="space-y-3">
          {faqs.map((faq, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="border border-charcoal/10 rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm"
            >
              <button
                onClick={() => setOpen(open === i ? null : i)}
                className="w-full text-left px-6 py-5 flex items-center justify-between gap-4 group"
              >
                <span className="text-base sm:text-lg font-bold text-charcoal font-signika leading-snug group-hover:text-pumpkin transition-colors">
                  {faq.q}
                </span>
                <motion.span
                  animate={{ rotate: open === i ? 45 : 0 }}
                  transition={{ duration: 0.25 }}
                  className="flex-shrink-0 w-6 h-6 rounded-full border border-charcoal/20 flex items-center justify-center text-charcoal/60 group-hover:border-pumpkin group-hover:text-pumpkin transition-colors"
                >
                  +
                </motion.span>
              </button>
              ...
              <AnimatePresence>
                {open === i && (
                  ...
                  >
                    <div className="px-6 pb-6 text-charcoal/70 font-alan-sans text-sm sm:text-base leading-relaxed border-t border-charcoal/5 pt-4">
                      {faq.a}
                    </div>
                  </motion.div>
                )}
        ...
        >
          <p className="text-charcoal/50 font-alan-sans text-sm mb-4">Still have questions?</p>
          <a
            href="mailto:hello@theshaktcollective.com"
            className="inline-flex items-center gap-2 text-pumpkin font-bold font-alan-sans hover:gap-3 transition-all"
          >
            Reach out to us →
          </a>
        </motion.div>
