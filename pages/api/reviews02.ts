...
const HOLYSHEET_API_KEY = process.env.HOLYSHEET_API_KEY_02 || 'Ds0LWPH9IK2k3Kz0KTn147q54QZ-uk7a';
...
const isHeaderLikeIncomingRow = (row: Record<string, any>) => {
  const pairs: Array<[string, string[]]> = [
    ...
    ['Date', ['Date', 'date', 'submittedAt', 'submitted at']],
  ];
  ...
  try {
    if (req.method === 'GET') {
      ...
      const allReviews = rows.map((row: any, index: number) => {
        const numericRating = parseFloat(String(getField(row, ['RatingNumber', 'ratingnumber', 'rating']) || '5')) || 5;
        const approvedValue = getField(row, ['Approved', 'approved', 'isApproved', 'isapproved']);
        ...
        return {
          ...
          name:
            getField(row, ['Name', 'name']) ||
            `${getField(row, ['firstName', 'first name']) || ''} ${getField(row, ['lastName', 'last name']) || ''}`.trim() ||
            'Anonymous',
          title: getField(row, ['Title', 'title']) || 'Masterclass Review',
          content: getField(row, ['Content', 'content', 'oneLineExperience']) || getField(row, ['improvementSuggestion']) || '',
    ...
    if (req.method === 'POST') {
      ...
      const row = {
        ...
        RatingNumber: finalWeightedRating,
        Approved: false,
        firstName,
        lastName,
        registeredMobile,
