...
(function () {
  ...
  function ensureAccessibleControlNames() {
    var platformLabels = {
      ...
      'youtube.com': 'Open YouTube',
      'youtu.be': 'Open YouTube',
      'facebook.com': 'Open Facebook',
      'linkedin.com': 'Open LinkedIn',
      'wa.me': 'Open WhatsApp',
      'api.whatsapp.com': 'Open WhatsApp',
      'open.spotify.com': 'Open Spotify'
    };
    document.querySelectorAll('a[href]').forEach(function (anchor) {
      if (anchor.getAttribute('aria-hidden') === 'true') return;
      var currentName = (anchor.getAttribute('aria-label') || anchor.getAttribute('title') || anchor.textContent || '').trim();
      if (currentName) return;
      var image = anchor.querySelector('img[alt]');
      var imageAlt = image && (image.getAttribute('alt') || '').trim();
      if (imageAlt) {
      ...
      try {
        ...
        var hostLabel = Object.keys(platformLabels).find(function (host) {
          return url.hostname === host || url.hostname.endsWith('.' + host);
        });
        if (hostLabel) {
          anchor.setAttribute('aria-label', platformLabels[hostLabel]);
          return;
        }
        var pathLabel = decodeURIComponent(url.pathname.split('/').filter(Boolean).pop() || 'linked page').replace(/[-_]+/g, ' ');
        anchor.setAttribute('aria-label', 'Open ' + pathLabel);
      } catch (e) {
        anchor.setAttribute('aria-label', 'Open linked page');
      }
    });
    document.querySelectorAll('button').forEach(function (button) {
      var currentName = (button.getAttribute('aria-label') || button.getAttribute('title') || button.textContent || '').trim();
      if (!currentName && /comp-kkmqi5tc/.test(button.id || '')) {
  ...
  function mountYugmBandCardToggles(path) {
    ...
    var cards = [
      {
        ...
        arrow: 'comp-mqjigv2l',
        bio: 'comp-mqjigv2e3',
        label: 'Toggle Abhishek biography',
    ...
    cards.forEach(function (cfg) {
      ...
      arrow.setAttribute('aria-label', cfg.label);
  ...
  function usesNativeWixNav(path) {
    ...
    if (['/course-bundle', '/affiliate-apply'].indexOf(path || canonicalPathname()) !== -1) return false;
    return Array.prototype.some.call(
      document.querySelectorAll('header:not(.tsc-desktop-site-header):not(.tsc-mobile-site-header), #SITE_HEADER, [data-testid="siteHeader"]'),
      function (header) {
  ...
  function mountHomeMobileClosingCta(path) {
    path = path || canonicalPathname();
    var isHome = path === '/' || path === '/home' || path === '/pages/home.html';
    var compact = window.matchMedia && window.matchMedia('(max-width: 1024px)').matches;
    var existing = document.querySelector('.tsc-home-mobile-closing-cta');
    if (!isHome || !compact) {
      if (existing && existing.parentNode) existing.parentNode.removeChild(existing);
      return;
    }
    ...
    section.className = 'tsc-home-mobile-closing-cta';
    section.setAttribute('aria-labelledby', 'tsc-home-mobile-closing-title');
    section.innerHTML = [
      '<div class="tsc-home-mobile-closing-cta__inner">',
      '<p class="tsc-home-mobile-closing-cta__eyebrow">Choose your next step</p>',
      '<h2 id="tsc-home-mobile-closing-title">Your artistry doesn’t need more noise. It needs clarity.</h2>',
      '<p>With the right mentorship, systems, and environment, growth starts compounding. Whether you want to learn, create, collaborate, or build something meaningful, there’s space for you here.</p>',
      '<div class="tsc-home-mobile-closing-cta__actions">',
      '<a href="/academy">Explore TSC Academy</a>',
      '<a href="/artist-path">Discover Artist Path</a>',
      '<a href="/films">Build With TSC</a>',
      '<a href="' + ARTIST_PATH_WHATSAPP + '" target="_blank" rel="noreferrer noopener">Join The Ecosystem</a>',
      '</div>',
      '</div>'
    ].join('');
    var footer = document.querySelector('.tsc-shared-footer-host');
  ...
  function mountAcademyMobileBundleCard(path) {
    path = path || canonicalPathname();
    if (path !== '/academy') return;
    mountAcademyMobileExperience(path);
    if (window.matchMedia && !window.matchMedia('(max-width: 1024px)').matches) return;
    if (document.querySelector('.tsc-academy-mobile-experience')) return;
  ...
  function mountDesktopHeader(opts) {
    ...
    var locked = forceCustomHeader || !usesNativeWixNav(config.path) ? null : activateLockedDesktopHeader();
    if (locked) {
